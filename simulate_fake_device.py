...
# @NOTE: How this stupid list came to be
...
# 3) Run the script and verify that address shows up on the demo fotaportal page.
...
def send(data):
    ...
    d = agent.request('PUT', WEBHOOK_URL, headers, body)

    def body_read(body, response):
        if response.code >= 400:
            raise ValueError("send failed with %s: %s" % (response.code, body))
        elif response.code >= 500:
            raise Exception("send failed on server with %s: %s" % (response.code, body))
